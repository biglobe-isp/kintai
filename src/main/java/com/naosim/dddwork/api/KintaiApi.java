...
import com.naosim.dddwork.datasource.CsvAttendanceRepository;
import com.naosim.dddwork.datasource.CsvRuleRepository;
...
public class KintaiApi {
    ...
    public KintaiApi() {
        // ここでいいの疑問をもっているが
        service = new KintaiService(new CsvAttendanceRepository(), new CsvRuleRepository());
    }
    ...
    public void Total(String yearMonth) {
        ...
        // サービス呼出し
        ResultTotal result = this.service.total(yearMonth);

        System.out.println("勤務時間: " + result.getTotalWorkMinutes() / 60 + "時間" + result.getTotalWorkMinutes() % 60 + "分");
        System.out.println("残業時間: " + result.getTotalOverWorkMinutes() / 60 + "時間" + result.getTotalOverWorkMinutes() % 60 + "分");
    }
